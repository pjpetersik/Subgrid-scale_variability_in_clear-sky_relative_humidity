...
!!   -# P. Petersik (ppetersik): added stochastic parameterization of subgrid variability of clear-sky RH for computing hygroscopic growth
!!
!! \limitations
!! None
!!
...
PUBLIC      :: m7_interface, m7_cumulative_normal


CONTAINS

! ---------------------------------------------------------------------------
!  m7_interface: called from mo_submodel_interface 
! ---------------------------------------------------------------------------

SUBROUTINE m7_interface(kproma, kbdim, klev, krow, ktrac, &
                        pap,    paph,                     &
                        pt,     pq,                       &
                        pxtm1,  pxtte,                    &
                        paclc,  pgrvolm1, ppbl)
  ...
  !   - added m7mode and new modules, removed dependency on mo_ham_m7_trac,
  ...
  ! quantities to m7. A mass conservation check is performed (before and after
  ! calling m7) if requested.
  !
  ! After the call of m7 the returning concentrations are reconverted
  ! to mixing ratios, the tendencies of the mixing ratios are calculated
  ! and returned to ECHAM.
  
  USE mo_kind,           ONLY: dp
  USE mo_constants,      ONLY: rd, vtmpc1, g, avo, argas
  USE mo_time_control,   ONLY: time_step_len
  USE mo_convect_tables, ONLY: tlucuaw, jptlucu1, jptlucu2
  USE mo_exception,      ONLY: finish, message, message_text, em_warn
  USE mo_species,        ONLY: speclist
  USE mo_ham_m7ctl,      ONLY: nsol, lmass_diag, lgcr,  &
                               nsolact,                                        &
                               m7_ngasspec, m7_gasspec, m7_gasunitconv,        &
                               m7_naerospec, m7_aerospec, m7_aerounitconv,     &
                               m7_aero_idx,                                    &
                               immr2ug, immr2molec, ivmr2molec
  USE mo_ham_m7_species, ONLY: im7_so4g
  USE mo_ham,            ONLY: naerocomp, aerocomp, aerowater, mw_so4, &
                               m7mode, nmod
...
! optional arguments
  ...
  REAL(dp):: zmass_pre(m7_naerospec),       & ! mass of aerosol compounds before and
             zmass_post(m7_naerospec)         ! after m7 call, for mass conservation check
  !<<dod
  ...
  !>>dod soa
  REAL(dp) :: zgas(kbdim,klev,m7_ngasspec)
  ...
  REAL(dp):: zaerml(kbdim,klev,naerocomp), & ! aerosol mass for individual compounds [molec. cm-3 for sulfate and ug m-3 for bc, oc, ss, and dust]
             zaernl(kbdim,klev,nmod),    & ! aerosol number for each mode [cm-3]
             zm6rp(kbdim,klev,nmod),     & ! mean mode actual radius (wet for soluble and dry for insoluble modes) [cm]
             zm6dry(kbdim,klev,nsol),    & ! dry radius for soluble modes [cm]
             zrhop(kbdim,klev,nmod),     & ! mean mode particle density [g cm-3]
             zww(kbdim,klev,nmod)          ! aerosol water content for each mode [kg(water) m-3(air)]
  ...
  CALL xt_borrow(kproma, kbdim,  klev, ilevp1, ktrac, krow, &
                 pap,    paph,                              &
                 pxtm1,  pxtte                               )
...
!!mgs(S)!!  zfacm  = 6.022e+20_dp/32._dp
  ...
  !--- 2) Calculate ambient properties: ----------------------------------------
    ...
    DO  jl = 1,kproma
      ...
      zdpg(jl,jk) = (paph(jl,jk+1)-paph(jl,jk))/g
...
!!mgs(S)!!  !           to [molecules/cm+3]
  ...
  !>>dod soa (note presently mmr2mmolec will only work for sulphate, needs update)
!++mgs: Note that logic changed here - looping only over m7_gasspec !! 
!! this is equivalent to the former "IF (gasspec(jn)%lm7gas) THEN"
!! the former "gasspec(jn)%m7unitconv" is now replaced with m7_gasunitconv(jn)
!--mgs
!++mgs: initialize zgso4p1
  zgso4p1(:,:) = 0._dp

  DO jn=1,m7_ngasspec
    jt = speclist(m7_gasspec(jn))%idt

    SELECT CASE(m7_gasunitconv(jn))
    CASE(immr2ug)
      DO jk=1,klev
        DO jl=1,kproma
  ...
  DO jn=1, naerocomp
     ...
     jl    = m7_aero_idx(jspec)     ! get index to m7_aerospec list

     IF (jl <= 0) THEN
       WRITE(message_text,'(a,i0)') 'M7_AERO_IDX Mapping error !! No index for jspec=',jspec
       CALL finish('m7_interface', message_text)
     END IF
  ...
  IF (lmass_diag) CALL m7_mass_sum(kbdim, kproma, klev, krow, zgas, zaerml, zdpg, zmass_pre)
  !<<dod

  !--- 4) Call of m7: ----------------------------------------------------------
  
  ! The gas phase concentrations at t-1 and their derivatives for processes
  ! preceeding m7 are passed to m7, as m7 performs their time integration
  ! together with the processes it accounts for. The aerosol tracers (mass and
  ! number) at t+1 after processes preceeding m7 are passed to m7, and m7
  ! applies its processes on these.
  ...
  zgas(1:kproma,:, im7_so4g) = zgso4(1:kproma,:)
...
!!mgs-old code!!     SELECT CASE(aerocomp(jn)%species%m7unitconv)    
     ...
     SELECT CASE(m7_aerounitconv(jl))
     CASE(immr2ug)                            
        zqunitfac = zqfac
     CASE(immr2molec)
        zqunitfac = zqfacm
  ...
  !--- 6) Convert M7 quantities to SI units and store in streams: --------------
...
!gf
  ...
  IF (lmass_diag) THEN
     ...
     CALL m7_mass_sum(kbdim, kproma, klev, krow, zgas, zaerml, zdpg, zmass_post)

     !--- Perform mass conservation check:
     !>>dod soa rewritten to avoid hardcoding of individual species
     DO jc=1,m7_naerospec

        IF( ABS(zmass_pre(jc)-zmass_post(jc)) > 0.100_dp*ABS(MAX(zmass_pre(jc),zmass_post(jc)))) THEN

           CALL message('m7_interface', 'm7 violates mass conservation by >10% for '// &
                        speclist(m7_aerospec(jc))%longname, level=em_warn)

           labort=.TRUE.        
        
        ELSE IF( ABS(zmass_pre(jc)-zmass_post(jc)) > 0.010_dp*ABS(MAX(zmass_pre(jc),zmass_post(jc)))) THEN

           CALL message('m7_interface', 'm7 violates mass conservation by >1% for '// &
                        speclist(m7_aerospec(jc))%longname)

        ELSE IF( ABS(zmass_pre(jc)-zmass_post(jc)) > 0.001_dp*ABS(MAX(zmass_pre(jc),zmass_post(jc)))) THEN

           CALL message('m7_interface', 'm7 violates mass conservation by >0.1% for '// &
                        speclist(m7_aerospec(jc))%longname)
        END IF
...
SUBROUTINE m7(kproma, kbdim,   klev, krow, &
  ...
  USE mo_kind,           ONLY: dp
  USE mo_ham_m7ctl,      ONLY: nsnucl, nonucl, lscoag, lscond, lnucl_stat,  &
                               nsol, nwater,                 &
                               m7_ngasspec
  USE mo_ham_m7_species, ONLY: im7_so4g
  USE mo_ham_nucl_diag,  ONLY: sample_nucl_statistics
  !>>dod soa
  ...
  !>>dod soa
  REAL(dp) :: pgas(kbdim,klev,m7_ngasspec)    ! gas phase tracers 
  !<<dod
  ...
  !>>dod soa
  pgas(1:kproma,:,im7_so4g) = pso4g(1:kproma,:)
  !<<dod
...
SUBROUTINE m7_averageproperties(kproma, kbdim, klev, krow, paernl, paerml, pttn, pm6rp, prhop)
  ...
  USE mo_ham_m7ctl,      ONLY: iso4ns,                                     &
                               critn,  ram2cmr,                            &
                               m7_aerounitconv, &
                               m7_aero_idx, immr2ug, cdconv, cmin_aernl, cmin_aerml, cminvol
  ...
  DO jn=1,naerocomp
     ...
     IF (jn == iso4ns) THEN
        WHERE (paernl(1:kproma,:,jclass) > cmin_aernl .AND. paerml(1:kproma,:,jn) > cmin_aerml)
  ...
  DO jclass=1,nmod
     IF (.NOT. m7mode(jclass)%lsoluble) THEN
        WHERE (zinsvol(1:kproma,:,jclass) > cminvol .AND. paernl(1:kproma,:,jclass) > cmin_aernl)
...
SUBROUTINE m7_kappa(kproma, kbdim, klev, krow, prelhum, paernl, pttn, ptp1, &
  ...
  USE mo_species,         ONLY: speclist
  USE mo_ham_m7ctl,       ONLY: nsol, &
                                cmr2ram, ram2cmr,   &
                                m7_aerounitconv,  &
                                m7_aero_idx, immr2molec, cdconv, cminvol, cminrad
  USE mo_ham,             ONLY: naerocomp, aerocomp, m7mode, nmod
  ...
  DO jn=1,naerocomp
     ...
     IF (m7mode(jclass)%lsoluble .AND. (speclist(jspec)%kappa > kappa_min)) THEN
        ...
        DO jk=1,klev
           DO jl=1,kproma
              IF (zvolh(jl,jk,jclass) > cminvol) THEN
     ...
     IF (m7mode(jclass)%lsoluble) THEN
        ...
        DO jk = 1,klev
           DO jl = 1,kproma
              ...
              IF (pm6dry(jl,jk,jclass) > cminrad) THEN
...
SUBROUTINE m7_equimix(kproma,  kbdim, klev, krow,  &
  ...
  USE mo_ham_m7ctl,    ONLY: nsol,                               &
                             dh2so4,  dh2o,                      &
                             wvb,     gmb,     wh2so4,  ram2cmr, cmin_aerml, cdconv 
  USE mo_species,      ONLY: speclist
  ...
  DO 50 jclass=iaccs, icoas
     ...
     DO jn=1,m7_naerospec     !!mgs!!     ham_naerospec
        IF ((.NOT. speclist(m7_aerospec(jn))%lelectrolyte) .AND. (jn /= im7_wat)) THEN
           jm = speclist(m7_aerospec(jn))%iaerocomp(jclass) 
           DO jk=1,klev
              DO jl=1,kproma
                 !--- mass in g m-3
                 ...
                 zothervol(jl,jk) = zothervol(jl,jk) + &
                                   1.e-6_dp*paerml(jl,jk,jm)/(cdconv*speclist(m7_aerospec(jn))%density)
              END DO
           END DO
        END IF
     END DO
   ...
   DO jk=1,klev
      DO jl=1,kproma
         IF (pm6rp(jl,jk,jclass).GT.0._dp) THEN                          ! try re-code avgprops so that this is never the case. (call 1 of avgprops)
            ...
            zvelb=SQRT(8.0_dp * rerg * ptp1(jl,jk) / api / wh2so4)       ! merge the constants. isolate sqrt(t) since it usually doesn't vectorise.
...
SUBROUTINE m7_prod_cond(kproma, kbdim,  klev,  krow, &
  ...
  USE mo_ham,           ONLY: naerocomp, mw_s, nmod
  ...
  DO jk=1,klev
    DO jl=1,kproma
      ...
      IF (pcs(jl,jk) > 1.0E-10_dp) THEN  ! Regular H2SO4 condensation sink of the aerosol
        ...
        ! converting [molec. cm-3] to [kg(S) m-2]:
        
        d_cond_so4(jl,krow) = d_cond_so4(jl,krow) &
         + (((zfcond_cf + zfcond_cy)*mw_s*1.E3_dp)/avo)*pdz(jl,jk)*zqtmst*delta_time
...
SUBROUTINE m7_nuck(kproma,  kbdim,  klev,   krow,          &
  ...
  USE mo_ham,          ONLY: naerocomp, mw_s
    ...
    DO jl = 1, kproma
      ...
      IF (znucrate > zeps) THEN
        ...
        d_nuc_so4(jl,krow) = d_nuc_so4(jl,krow) &
         + pa4delt(jl,jk,1)*mw_s*1.E+03_dp/avo*pdz(jl,jk)*zqtmst*delta_time
...
SUBROUTINE m7_dnum(kproma, kbdim,  klev,   krow,  &
  ...
  USE mo_kind,       ONLY: dp
  USE mo_ham_m7ctl,  ONLY: lscoag, ncoag
  USE mo_ham,        ONLY: naerocomp, nmod
  !>>dod timers
  USE mo_control,    ONLY: ltimer
...
SUBROUTINE m7_dconc(kproma, kbdim, klev, krow, paerml, paernl, pm6dry)
  ...
  USE mo_ham_m7ctl,   ONLY: sigmaln, crdiv,                &
                            ...
                            cmin_aernl, cminrad
  USE mo_constants,   ONLY: api, avo
  USE mo_ham,         ONLY: naerocomp, aerocomp, nsoa, lsoa, m7mode, nmod
...
!<<dod
  ...
  !>>dod soa
  USE mo_ham,       ONLY: lsoa, nsoa, naerocomp, nmod
  USE mo_ham_soa,   ONLY: soaprop
  !>>dod deleted isoa_ix
  USE mo_species,   ONLY: speclist
  !<<dod
...
END SUBROUTINE m7_concoag
  ...
  SUBROUTINE m7_delcoa(kproma,   kbdim,  klev,     krow, paerml,   &
    ...
    USE mo_kind,          ONLY: dp
    USE mo_time_control,  ONLY: time_step_len
    USE mo_ham_m7ctl,     ONLY: nsol, cmin_aernl, cmin_aerml,          & 
                                ...
                                icoas
                                !<<dod
    USE mo_ham,           ONLY: lsoa, nsoa, naerocomp, m7mode, nmod
    !>>dod deleted isoa_ix
    USE mo_ham_soa,       ONLY: soaprop
    !<<dod
    USE mo_species,       ONLY: speclist
...
SUBROUTINE m7_mass_sum(kbdim, kproma, klev, krow, pgas, paerml, pdpg, pmasssum)
    
  ! Purpose: sum the total mass per species over the gridpoints on one processor
  ! This subroutine is called if the namelist variables lsoa and lomassdiag are TRUE
  ! It is called twice, from m7_interface, once before and once after m7.
  ! It tests the total mass and throws an exception if mass conservation is violated

  !---inherited functions, types and data
  USE mo_kind,             ONLY: dp
  USE mo_ham,              ONLY: naerocomp, aerocomp, lsoa, nsoa
  USE mo_ham_m7ctl,        ONLY: m7_ngasspec, m7_naerospec
  USE mo_ham_m7_species,   ONLY: im7_so2, im7_so4, im7_so4g
  ...
  REAL(dp), INTENT(IN) :: pgas(kbdim,klev,m7_ngasspec)        ! gas concentrations
  ...
  REAL(dp), INTENT(OUT) :: pmasssum(m7_naerospec)             ! total mass
...
!!mgs(S)!!: ### update needed to obtain correct mass??
    ...
    DO jk=1,klev
       DO jl=1,kproma
          pmasssum(im7_so4) = pmasssum(im7_so4) + (pgas(jl,jk,im7_so2)+pgas(jl,jk,im7_so4g)) * &
                                                pdpg(jl,jk)*zarea(jl)
       END DO
    END DO
